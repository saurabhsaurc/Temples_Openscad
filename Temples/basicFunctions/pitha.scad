...
module pitha(temple_profile,temple_mid_profile, temple_center_offset,height_array, size_array, diagonal_array)
{
    //Inputs:
    ...
    //Cylinder array - specifies if we should have cylindrical edge for that section - not used
    ...
    for(i=[0:len(height_array)-1])
    {
        ...
        /*
        if(cylinder_array[i]==1)
        {
            $fn=50;
            minkowski(){
            translate([0,0,cumulative_height(height_array,i-1)+height_array[i]/2])
            linear_extrude(height = 0.1)
            translate(temple_center_offset*size_array[i])
            polygon(points=temple_profile*size_array[i]);  
                
            sphere(r=height_array[i]/2);      
            }
        }*/
